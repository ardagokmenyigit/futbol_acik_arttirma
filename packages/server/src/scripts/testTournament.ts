...
console.log('Katılımcılar:', filled4.map((t) => `${t.nickname} (${t.isBot ? 'Bot' : 'Gerçek'})`));
...
t4.results.forEach((r, idx) => {
  ...
  console.log(`  Maç ${idx + 1}: ${home} ${r.scoreHome} - ${r.scoreAway} ${away}${pen} -> Tur Atlayan: ${winner}`);
});
...
console.log('Katılımcılar:', filled8.map((t) => `${t.nickname} (${t.isBot ? 'Bot' : 'Gerçek'})`));
